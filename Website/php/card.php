...
if ($imageSize1 <= 100 * 1024 && $imageSize2 <= 100 * 1024) {
    ...
    if ($result->num_rows > 0) {
        $show = '<h2>Die Karte existiert bereits. Wähle einen anderen Namen.</h2>';
    } else {
        // Der SQL-Befehl für das Hinzufügen
        $sql = "INSERT INTO $cardTable(name, bild, artist) VALUES('$name', 0x" . bin2hex($blob1) . ", 0x" . bin2hex($blob2) . ")";

        if (!$conn->query($sql)) {
            $show = '<h2>Einfügen fehlgeschlagen: ' . $conn->error . '</h2>';
        } else {
            $show = '<h2>Die Karte ' . $name . ' wurde erfolgreich hinzugefügt!</h2>';
        }
    }
} else {
    $show = '<h2>Das Bild darf maximal 100 KiB groß sein.</h2>';
}
